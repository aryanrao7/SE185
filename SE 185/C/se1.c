...
int coordinate[40];
//WINDOW *myWindow=newwin(5,65,44,2);
//mvwprintw(myWindow,44,2,"%s",intro);


//mvprintw(45, 2, "%s",myString);
while(y<40){
...
mvprintw(y,x,"%s", wordsStore[choosenWord]);
...
mvprintw(i+2,coordinate[i-1]+j,"a");
//mvprintw(48, 2, "%d, %d",i+2,coordinate[i]+j);
}
}
}
/*if(((seconds - myTime)%10 == 0) && myString == " "){
continue;
}
else{
mvprintw(44,2,"%s",intro);
